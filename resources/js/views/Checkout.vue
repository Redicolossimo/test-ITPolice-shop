                ...
                <div>
                    ...
                    <div v-if="isLoggedIn">
                        <div class="row">
                            ...
                            <label for="address" class="col-md-3 col-form-label mt-3">Telegram ID</label>
                            <div class="col-md-9 mt-3">
                                <input id="telegram_user_id" type="text" class="form-control" v-model="telegram_user_id">
                                <p>Enter your Telegram ID for notification</p>
                            </div>
                        </div>
                        <br>
...
export default {
    ...
    mounted() {
        this.isLoggedIn = localStorage.getItem('e-shop.jwt') != null
    },
    beforeMount() {
        axios.get(`/api/products/${this.pid}`).then(response => this.product = response.data)

        if (localStorage.getItem('e-shop.jwt') != null) {
            this.user = JSON.parse(localStorage.getItem('e-shop.user'))
            axios.defaults.headers.common['Content-Type'] = 'application/json'
            axios.defaults.headers.common['Authorization'] = 'Bearer ' + localStorage.getItem('e-shop.jwt')
        }
    },
    methods : {
        login() {
        ...
        placeOrder(e) {
            ...
            let telegram_user_id = this.telegram_user_id



            axios.post('api/orders/', {address, quantity, product_id, telegram_user_id})
